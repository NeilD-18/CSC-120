...
public class PokerHand {
    ...
    private ArrayList<Card> pokerHandContents = new ArrayList<Card>();
    ...
    public Card get_ith_card(int index) { 
        if (index >= FIRSTVALIDCARDINDEX && index < MAXCARDSINPOKERHAND) { 
    ...
    public String toString() { 
        ...
        for (int i = FIRSTVALIDCARDINDEX; i < size(); i++) {
            stringVersionOfPokerHand += get_ith_card(i).toString() + "\n";
        }
   ...
   private boolean isFlush(){
        Card firstCard = get_ith_card(FIRSTVALIDCARDINDEX); 
   ...
   private int[] getRanksOfHandInArray() { 
       ...
       for (int i = FIRSTVALIDCARDINDEX; i < size(); i++) { 
            ranksOfHandInArray[i] = get_ith_card(i).getRank();
       }
